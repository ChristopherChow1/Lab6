...
public abstract class Shape implements Comparable<Shape>
{
	...
	{
		if (Shape.this.getArea() > s.getArea()) {
			return 1;
		} else if (Shape.this.getArea() < s.getArea()) {
			return -1;
		} else {
			if (Shape.this.getPerimeter()> s.getPerimeter()) {
				return 1;
			} else if (Shape.this.getPerimeter() < s.getPerimeter()) {
				return -1;
			} else {
				return 0;
			}
	...
	public String toString()
	{
		...
		return String.format("%s \t ID = %s\t area = .3%f", getShapeType(), getId(), getArea());
	}
}
